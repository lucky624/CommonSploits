...
def put():
    ...
    if vuln == 1:
        ...
        try:
            response = s.post('http://{}:{}/api/login'.format(ip, port), json={'username': username, 'password': password},
                          timeout=(CT, RT)).text
        ...
        try:
            response = s.post('http://{}:{}/api/add'.format(ip, port),
                          json={'name': fake_name, 'description': fake_description, 'cost': fake_cost, 'in_stock': fake_in_stock},
                          timeout=(CT, RT)).text
            if not 'Ok' in response:
                print("Can't add merch")
                close(CORRUPT)
        ...
        try:
            response = s.post('http://{}:{}/api/login'.format(ip, port), json={'username': sender, 'password': password},
                          timeout=(CT, RT)).text
            if not 'User' in response:
                print("Can't login")
                close(CORRUPT)
